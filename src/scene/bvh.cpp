...
namespace CGL {
	namespace SceneObjects {
		...
		BVHNode* BVHAccel::construct_bvh(std::vector<Primitive*>::iterator start,
			...
			int axis = 0;
			if (bbox.extent[1] > bbox.extent[axis])
				axis = 1;
			...
			node->end = end;
			if (left > 0) {
				node->l = construct_bvh(start, start + left, max_leaf_size);
				node->r = construct_bvh(start + left, end, max_leaf_size);
			}
			return node;
